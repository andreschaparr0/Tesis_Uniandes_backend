...
\end{figure}\subsection{API REST: endpoints y tecnologías}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Figura_Services.png}
...
\begin{figure}[H]
    ...
    \label{fig:er-modelo}
\end{figure}
Profundizando en el componente de datos presentado en la Figura \ref{fig:capa_de_datos_en_backend}, la implementación sigue un diseño que desacopla la definición de las entidades de su lógica de manipulación. Como se detalla en la Figura \ref{fig:DetallesImplementacionDatos}, se emplean clases de modelo (\textit{Models}) que heredan de la base declarativa de SQLAlchemy para mapear atributos a columnas de la base de datos, mientras que los repositorios actúan como gestores que orquestan las consultas y transacciones.

El flujo de persistencia, ilustrado en la Figura \ref{fig:flujo_persistencia_analysis}, sigue una secuencia rigurosa para asegurar la consistencia. Cuando un servicio invoca un método de creación, el repositorio transforma los datos de entrada en una instancia del modelo ORM, gestiona la sesión de la base de datos para añadir el registro y ejecuta la confirmación (\textit{commit}). Finalmente, refresca la instancia para obtener datos generados por el motor (como el ID o fechas de creación) y la retorna a la capa de servicio, completando así el ciclo de almacenamiento de manera transparente para el resto de la aplicación.
