...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Fase_Comparacion_General_Pipeline_comparacion.png}
...
En la Figura \ref{fig:fase2-comparacion} se puede observar dos casos especiales donde el sistema aplica reglas adicionales para no perder señales relevantes.

En \textbf{certificaciones}, cuando la descripción de trabajo no declara certificaciones obligatorias pero sí define habilidades técnicas, se activa una lógica: si el CV tampoco tiene certificaciones o el Job no tiene habilidades técnicas, el aspecto se marca como ignorado; pero si el candidato sí reporta certificaciones y el Job sí define habilidades técnicas, las certificaciones del CV se comparan semánticamente contra esas habilidades técnicas mediante un prompt específico. Este flujo de decisiones se resume en la Figura \ref{fig:placeholder}, y esta logica se implemento dado una recomendación del evaluador RR.HH. del modelo que se hablara en secciones mas adelante.
...
\subsection{Rol del frontend en el sistema}
\noindent La interfaz web constituye el punto de entrada principal para los usuarios del sistema (reclutadores, analistas o investigadores). Desde el navegador, el usuario carga hojas de vida y descripciones de trabajo, configura los parámetros básicos del proceso de evaluación y consulta los resultados de compatibilidad generados por el motor de estructuración y comparación descrito en las secciones anteriores. De esta forma, el frontend actúa como una capa de presentación que abstrae la complejidad del procesamiento con modelos de lenguaje y expone un flujo de trabajo simple y guiado.

\subsection{Arquitectura general y comunicación con el backend}
\noindent El frontend se implementa como una aplicación web que se comunica con la API REST de FastAPI mediante peticiones HTTP que intercambian datos en formato JSON. Cada acción relevante del usuario (por ejemplo, subir un CV, registrar una descripción de trabajo o solicitar un análisis de compatibilidad) se traduce en llamadas a endpoints específicos del backend, que activan los pipelines de estructuración y de comparación documentados previamente. El estado de la aplicación (archivos cargados, resultados obtenidos, errores) se mantiene en el navegador, actualizando dinámicamente la interfaz en función de las respuestas del servidor.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Arquitectura_Frontend_Backend.png}
    \caption{Arquitectura general cliente-servidor entre frontend y backend}
    \label{fig:frontend-arquitectura}
\end{figure}

\noindent La Figura \ref{fig:frontend-arquitectura} ilustra este esquema cliente-servidor: el navegador envía solicitudes al backend, el cual orquesta los motores de estructuración y comparación, y finalmente retorna al frontend un JSON con scores y explicaciones que luego se transforman en visualizaciones comprensibles para el usuario.

\subsection{Vistas y flujos principales}
\noindent A nivel de interacción, la aplicación se organiza en un pequeño conjunto de vistas que reflejan el flujo natural de uso: cargar información, ejecutar el análisis y revisar los resultados. Esto permite que el usuario recorra el proceso de forma secuencial, con validaciones y mensajes claros en cada paso.

\subsubsection{Carga de hojas de vida y descripciones de trabajo}
\noindent En la primera vista, el usuario puede seleccionar y cargar uno o varios archivos de hoja de vida (en formato PDF) y registrar la descripción de trabajo correspondiente, ya sea pegando el texto o subiendo un archivo. La interfaz realiza una validación básica (formatos permitidos, campos obligatorios) antes de enviar los datos al backend, mostrando indicadores de progreso mientras se ejecutan los pipelines de extracción y limpieza de texto.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Vista_Carga_Documentos.png}
    \caption{Vista de carga de hojas de vida y descripciones de trabajo}
    \label{fig:frontend-carga}
\end{figure}

\subsubsection{Ejecución del análisis y configuración básica}
\noindent Una vez cargados los documentos, el usuario puede iniciar el proceso de análisis desde una vista de resumen donde se muestran los CVs y la descripción de trabajo seleccionados. En esta pantalla se pueden exponer opciones simples de configuración, como la selección del puesto objetivo o el ajuste de algunos parámetros de evaluación (por ejemplo, habilitar o deshabilitar ciertos aspectos de comparación). Al confirmar, el frontend envía la solicitud al endpoint correspondiente y presenta un estado de ``procesando'' hasta recibir la respuesta del motor de comparaciones.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Vista_Ejecucion_Analisis.png}
    \caption{Vista de ejecución del análisis de compatibilidad}
    \label{fig:frontend-analisis}
\end{figure}

\subsubsection{Visualización de resultados y explicaciones}
\noindent La vista final se centra en presentar el score de compatibilidad global junto con el desglose por aspecto (experiencia, habilidades, educación, certificaciones, idiomas, ubicación y responsabilidades). Además de los valores numéricos, se muestran las explicaciones generadas por los modelos de lenguaje para que el usuario pueda entender por qué un candidato fue mejor evaluado en ciertos aspectos que en otros. Esta pantalla sirve como herramienta de apoyo a la decisión, permitiendo comparar rápidamente distintos candidatos frente a una misma vacante.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Vista_Resultados_Comparacion.png}
    \caption{Vista de resultados y desglose por aspecto}
    \label{fig:frontend-resultados}
\end{figure}

\subsection{Manejo de errores y mensajes al usuario}
\noindent Dado que el frontend está acoplado a procesos intensivos de procesamiento de lenguaje natural, se diseñó un manejo explícito de errores y estados intermedios. Cuando se produce un fallo en la carga de archivos, un error de validación o un problema en la comunicación con la API, la interfaz muestra mensajes claros y accionables, evitando que el usuario quede en un estado de incertidumbre. Asimismo, durante la ejecución de los pipelines se utilizan indicadores de progreso y estados de ``cargando'' para informar que el sistema sigue trabajando en segundo plano.

\subsection{Consideraciones de usabilidad y diseño}
\noindent Por último, el diseño del frontend prioriza la simplicidad y la transparencia. Las vistas se organizan en un flujo lineal (carga \(\rightarrow\) análisis \(\rightarrow\) resultados), se evita sobrecargar al usuario con opciones avanzadas y se hace énfasis en mostrar no solo el score numérico, sino también las razones detrás de cada decisión. Esta combinación de visualización clara y explicaciones detalladas busca que el sistema sea útil tanto en contextos operativos de selección de personal como en escenarios de análisis académico y prototipado de motores de recomendación basados en modelos de lenguaje.