...
\section{Motor de Estructuración y Procesamiento de Lenguaje Natural}
...
\subsection{Pipeline de procesamiento de hojas de vida}
...
\end{minipage}
\end{samepage}
\subsubsection{Fase 1: Extracción y limpieza de texto}
...
\begin{itemize}
    \item \textbf{Role Prompting:} Se instruye al modelo explícitamente: \textit{"Eres un experto en extraer información estructurada de hojas de vida"}. Esta técnica, identificada como el patrón de Persona \cite{white2023prompt}, condiciona la atención del modelo a entidades y terminología relevantes del dominio de reclutamiento.
    \item \textbf{Structured Output Prompting:} Se define un esquema JSON rígido dentro del prompt y se prohíbe la generación de texto conversacional adicional. Esta restricción es fundamental para garantizar la interoperabilidad con el sistema backend \cite{liu2023pre}.
    \item \textbf{Zero-Shot Prompting:} No se proveen ejemplos de CVs previos en el contexto; el modelo opera basándose únicamente en las instrucciones semánticas y el esquema objetivo, aprovechando su capacidad de generalización pre-entrenada \cite{brown2020language}.
\end{itemize}

A continuación se presenta un ejemplo real de la estructura de prompt utilizada para la extracción de información personal, tal como se implementa en la clase `SimpleCVExtractor`:
...
\begin{quotation}
    ...
    \end{verbatim}
    Si no encuentras alguna información, deja el campo como DESCONOCIDO. Texto del CV: [CONTENIDO\_DEL\_PDF]"
\end{quotation}
...
\subsubsection{Fase 3: Consolidación y validación de datos}
